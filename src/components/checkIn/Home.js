...
import FlightDetails from "./FlightDetails_old";
...
class Home extends React.Component {
  componentDidMount() {
    const { flights, actions, selectedFlight } = this.props;
    if (flights.length === 0) {
    ...
    this.setState({ selectedFlight }, () => {
      console.log("Selected flight id is", this.state.selectedFlight);
    });
  }
...
function mapStateToProps(state) {
  return {
    ...
    selectedFlight:
      state.selectedFlight === undefined ? 0 : state.selectedFlight,
  };
}
